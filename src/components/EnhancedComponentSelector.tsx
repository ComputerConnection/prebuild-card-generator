...
export function EnhancedComponentSelector({
  ...
  const filteredComponents = useMemo(() => {
    ...
    return filteredByBrand.filter(comp =>
  ...
  const addToRecent = (name: string) => {
    ...
    newRecent[category] = [
      name,
      ...newRecent[category].filter(r => r !== name)
    ].slice(0, MAX_RECENT);
    ...
    return (
      ...
      >
        <div className="flex-1 min-w-0">
          <p className={`text-sm truncate ${isSelected ? 'font-semibold text-blue-700' : 'text-gray-800'}`}>
            {name}
          </p>
        </div>
        <button
  ...
  return (
    <div className="mb-3 relative" ref={dropdownRef}>
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          type="button"
          onClick={() => { setShowAddForm(!showAddForm); setIsOpen(true); }}
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-80 overflow-hidden" style={{ maxWidth: 'calc(100% - 2rem)' }}>
          {/* Add Form */}
          {showAddForm && (
          ...
          <div className="overflow-y-auto max-h-52">
            ...
            {/* All components */}
            <div>
              {(searchQuery || selectedBrand || (categoryFavorites.length === 0 && categoryRecent.length === 0)) && (
                <div className="px-3 py-1.5 bg-gray-100 text-xs font-medium text-gray-600">
                  {searchQuery ? `Results for "${searchQuery}"` : selectedBrand ? selectedBrand : 'All Components'}
